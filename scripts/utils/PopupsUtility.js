...
export default class PopupsUtility {
    ...
    showOverlay (){
        console.log('PopupsUtility.showOverlay() called');
        this.popupOverlay.setVisible(true);
        this.popupVisible = true;
    }
    hideOverlay (){
        console.log('PopupsUtility.hideOverlay() called');
    ...
    debugTypesPopup()
    {
        ...
        // Create the panel graphics using a rectange with stroke
        const panelBackground = this.scene.add.graphics({ lineStyle: { width: 3, color: 0x0a0a0a }, fillStyle: { color: 0x161616 } });
        panelBackground.strokeRect(panelCenterX, panelY, panelWidth, panelHeight);
        panelBackground.fillRect(panelCenterX, panelY, panelWidth, panelHeight);

        // Loop through all the types and list them in the panel
        ...
        // Add the types text to the panel
        let panelText = typesText;
        const $panelText = this.scene.add.text(textPositionX, textPositionY, panelText, {
            fontSize: 16,
            fontFamily: 'Open Sans',
            lineSpacing: 10,
            align: 'left',
